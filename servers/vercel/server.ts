...
const config = await metorial.getArgs() as { token: string };
...
server.registerTool(
  'create_project',
  {
    ...
    inputSchema: {
      ...
      framework: z.string().optional().describe('Framework preset (e.g., nextjs, vite, remix)'),
      ...
      }).optional().describe('Git repository configuration'),
      ...
      })).optional().describe('Environment variables to add')
    }
  },
...
server.registerTool(
  ...
  async ({ deploymentId, teamId }) => {
    ...
    const endpoint = `/v12/deployments/${deploymentId}/cancel${params.toString() ? '?' + params.toString() : ''}`;
    const response = await vercelRequest<any>(endpoint, {
      method: 'PATCH'
    });
...
server.registerTool(
  ...
  async ({ deploymentId, teamId, limit }) => {
    ...
    const endpoint = `/v2/deployments/${deploymentId}/events${params.toString() ? '?' + params.toString() : ''}`;
    const response = await vercelRequest<any>(endpoint);
...
server.registerTool(
  ...
  async ({ projectId, teamId }) => {
    ...
    let endpoint: string;
    if (projectId) {
      endpoint = `/v9/projects/${projectId}/domains${params.toString() ? '?' + params.toString() : ''}`;
    } else {
...
server.registerTool(
  ...
  async ({ projectId, domain, teamId }) => {
    ...
    const endpoint = `/v10/projects/${projectId}/domains${params.toString() ? '?' + params.toString() : ''}`;
    const response = await vercelRequest<any>(endpoint, {
      method: 'POST',
...
server.registerTool(
  ...
  async ({ domain, teamId }) => {
    ...
    const endpoint = `/v6/domains/${domain}${params.toString() ? '?' + params.toString() : ''}`;
    await vercelRequest<any>(endpoint, {
      method: 'DELETE'
    });
...
server.registerTool(
  ...
  async ({ domain, teamId }) => {
    ...
    const endpoint = `/v6/domains/${domain}/verify${params.toString() ? '?' + params.toString() : ''}`;
    const response = await vercelRequest<any>(endpoint, {
      method: 'POST'
    });
...
server.registerTool(
  ...
  async ({ projectId, teamId }) => {
    ...
    const endpoint = `/v9/projects/${projectId}/env${params.toString() ? '?' + params.toString() : ''}`;
    const response = await vercelRequest<any>(endpoint);
...
server.registerTool(
  ...
  async ({ projectId, key, value, target, gitBranch, teamId }) => {
    ...
    const endpoint = `/v10/projects/${projectId}/env${params.toString() ? '?' + params.toString() : ''}`;
    const response = await vercelRequest<any>(endpoint, {
      method: 'POST',
      body: JSON.stringify(body)
    });
...
server.registerTool(
  'update_env_variable',
  {
    ...
    inputSchema: {
      ...
      target: z.array(z.enum(['production', 'preview', 'development'])).optional().describe('Target environments'),
      gitBranch: z.string().optional().describe('Git branch to scope the variable to'),
      teamId: z.string().optional().describe('Team ID')
    }
  },
  async ({ projectId, envId, value, target, gitBranch, teamId }) => {
    ...
    const endpoint = `/v9/projects/${projectId}/env/${envId}${params.toString() ? '?' + params.toString() : ''}`;
    const response = await vercelRequest<any>(endpoint, {
      method: 'PATCH',
      body: JSON.stringify(body)
    });
...
server.registerTool(
  ...
  async ({ deploymentId, alias, teamId }) => {
    ...
    const endpoint = `/v2/deployments/${deploymentId}/aliases${params.toString() ? '?' + params.toString() : ''}`;
    const response = await vercelRequest<any>(endpoint, {
      method: 'POST',
