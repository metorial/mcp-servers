...
/**
 * Retrieve configuration from Metorial
 */
const config = await metorial.getArgs<Config>();
...
server.registerTool(
  'perplexity_ask',
  {
    ...
    description: 'Engages in a conversation using the Sonar API. Accepts an array of messages (each with a role and content) and returns a chat completion response from the Perplexity model with citations.',
    inputSchema: {
      messages: z.array(
        z.object({
...
server.registerTool(
  'perplexity_research',
  {
    ...
    description: 'Performs deep research using the Perplexity API. Accepts an array of messages and returns a comprehensive research response with citations using the sonar-deep-research model.',
    inputSchema: {
      messages: z.array(
        z.object({
          role: z.string().describe('Role of the message (e.g., system, user, assistant)'),
...
server.registerTool(
  'perplexity_reason',
  {
    ...
    description: 'Performs reasoning tasks using the Perplexity API. Accepts an array of messages and returns a well-reasoned response using the sonar-reasoning-pro model.',
    inputSchema: {
      messages: z.array(
        z.object({
          role: z.string().describe('Role of the message (e.g., system, user, assistant)'),
...
server.registerTool(
  'perplexity_search',
  {
    ...
    description: 'Performs web search using the Perplexity Search API. Returns ranked search results with titles, URLs, snippets, and metadata.',
    inputSchema: {
      query: z.string().describe('Search query string'),
      max_results: z.number().min(1).max(20).optional().describe('Maximum number of results to return (1-20, default: 10)'),
      max_tokens_per_page: z.number().min(256).max(2048).optional().describe('Maximum tokens to extract per webpage (default: 1024)'),
      country: z.string().optional().describe('ISO 3166-1 alpha-2 country code for regional results (e.g., "US", "GB")')
    }
  },
      ...
      contents: [
        {
          ...
          text: JSON.stringify({
            id: conversationId,
            message: 'Conversation history not available - Perplexity API does not provide conversation storage. Consider implementing local caching if needed.'
          }, null, 2),
...
server.registerResource(
  ...
  async (uri, { citationUrl }) => {
    const decodedUrl = decodeURIComponent(citationUrl);
    return {
      contents: [
        {
          uri: uri.href,
...
server.registerResource(
  ...
  async (uri, { topic }) => {
    try {
      ...
      throw new Error(`Failed to fetch research: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
);
