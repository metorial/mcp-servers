...
function formatItem(item: HNItem): string {
  ...
  if (item.kids && item.kids.length > 0) {
    lines.push(`\nChild IDs (${item.kids.length}): ${item.kids.slice(0, 10).join(', ')}${item.kids.length > 10 ? '...' : ''}`);
  }
...
function formatUser(user: HNUser): string {
  ...
  if (user.submitted && user.submitted.length > 0) {
    ...
    lines.push(`Recent IDs: ${user.submitted.slice(0, 20).join(', ')}${user.submitted.length > 20 ? '...' : ''}`);
  }
...
server.registerTool(
  'get_top_stories',
  {
    ...
    inputSchema: {
      limit: z.number().min(1).max(500).optional().describe('Maximum number of story IDs to return (default: 30)')
    }
  },
  async ({ limit = 30 }) => {
...
server.registerTool(
  'get_ask_stories',
  {
    ...
    inputSchema: {
      limit: z.number().min(1).max(200).optional().describe('Maximum number of story IDs to return (default: 30)')
    }
  },
  async ({ limit = 30 }) => {
...
server.registerTool(
  'get_job_stories',
  {
    ...
    inputSchema: {
      limit: z.number().min(1).max(200).optional().describe('Maximum number of job IDs to return (default: 30)')
    }
  },
  async ({ limit = 30 }) => {
...
server.registerTool(
  'get_user_submissions',
  {
    ...
    inputSchema: {
      ...
      limit: z.number().min(1).max(100).optional().describe('Maximum number of submission IDs to return (default: 30)')
    }
  },
  async ({ username, limit = 30 }) => {
    ...
    return {
      content: [
        {
          type: 'text' as const,
          text: `User "${username}" submissions (${limitedIds.length} of ${user.submitted.length} total):\n${limitedIds.join('\n')}\n\nUse the hn://item/{id} resource to get details about each submission.`
        }
      ]
    };
  }
...
server.registerTool(
  'get_max_item_id',
  {
    ...
    description: 'Get the current maximum item ID from Hacker News (useful for polling for new items)',
    inputSchema: {}
  },
  async () => {
...
server.registerTool(
  ...
  async () => {
    const updates = await fetchHN<{ items: number[]; profiles: string[] }>('/updates.json');

    return {
      content: [
        {
          type: 'text' as const,
          text: `Recent updates:\n\nUpdated Items (${updates.items.length}):\n${updates.items.slice(0, 20).join('\n')}${updates.items.length > 20 ? '\n...' : ''}\n\nUpdated Profiles (${updates.profiles.length}):\n${updates.profiles.slice(0, 20).join('\n')}${
...
server.registerResource(
  ...
  new ResourceTemplate('hn://item/{id}', { list: undefined }),
  {
    ...
    description: 'Get detailed information about any Hacker News item (story, comment, poll, job, etc.)'
  },
  async (uri, { id }) => {
    const item = await fetchHN<HNItem>(`/item/${id}.json`);
...
server.registerResource(
  ...
  new ResourceTemplate('hn://poll/{id}', { list: undefined }),
  {
    ...
    description: 'Get detailed information about a Hacker News poll including all poll options'
  },
  async (uri, { id }) => {
