...
const config = await metorial.getArgs<{ token: string }>();
...
server.registerResource(
  'payment_intent',
  new ResourceTemplate('stripe://payment-intents/{payment_intent_id}', { list: undefined }),
  {
...
server.registerTool(
  'list_customers',
  {
    ...
    inputSchema: {
      ...
      limit: z.number().optional().default(10).describe('Number of customers to return (1-100)'),
...
server.registerTool(
  'create_customer',
  {
    ...
    inputSchema: {
      ...
      metadata: z.record(z.string()).optional().describe('Set of key-value pairs for custom metadata')
    }
  },
...
server.registerTool(
  'list_payment_intents',
  {
    ...
    inputSchema: {
      ...
      limit: z.number().optional().default(10).describe('Number of payment intents to return (1-100)'),
...
server.registerTool(
  ...
  async ({ amount, currency, customer, description, payment_method, confirm, metadata }) => {
    ...
    return {
      content: [
        {
          type: 'text',
          text: `Payment intent created successfully: ${paymentIntent.id}\n\n${JSON.stringify(paymentIntent, null, 2)}`
        }
      ]
    };
  }
...
server.registerTool(
  ...
  async ({ payment_intent_id, amount, currency, customer, description, metadata }) => {
    ...
    const paymentIntent = await stripeRequest(`/payment_intents/${payment_intent_id}`, 'POST', body);
    return {
      content: [
        {
          type: 'text',
...
server.registerTool(
  ...
  async ({ payment_intent_id, payment_method }) => {
    ...
    const paymentIntent = await stripeRequest(`/payment_intents/${payment_intent_id}/confirm`, 'POST', body);
    return {
      content: [
        {
          type: 'text',
...
server.registerTool(
  ...
  async ({ payment_intent_id }) => {
    const paymentIntent = await stripeRequest(`/payment_intents/${payment_intent_id}/cancel`, 'POST');
    return {
      content: [
        {
          type: 'text',
...
server.registerTool(
  'capture_payment_intent',
  {
    ...
    inputSchema: {
      ...
      amount_to_capture: z.number().optional().describe('Amount to capture in cents (defaults to full amount)')
    }
  },
  async ({ payment_intent_id, amount_to_capture }) => {
    ...
    const paymentIntent = await stripeRequest(`/payment_intents/${payment_intent_id}/capture`, 'POST', body);
    return {
      content: [
        {
          type: 'text',
...
server.registerTool(
  'list_charges',
  {
    ...
    inputSchema: {
      ...
      limit: z.number().optional().default(10).describe('Number of charges to return (1-100)'),
...
server.registerTool(
  ...
  {
    title: 'Create Charge',
    description: 'Create a direct charge (legacy API, prefer Payment Intents for new integrations)',
    inputSchema: {
      amount: z.number().describe('Amount in cents (e.g., 1000 for $10.00)'),
      currency: z.string().describe('Three-letter ISO currency code (e.g., usd, eur)'),
      customer: z.string().optional().describe('Customer ID'),
...
server.registerTool(
  'list_subscriptions',
  {
    ...
    inputSchema: {
      ...
      limit: z.number().optional().default(10).describe('Number of subscriptions to return (1-100)'),
...
server.registerTool(
  'update_subscription',
  {
    ...
    inputSchema: {
      ...
      items: z.array(z.object({
        id: z.string().optional().describe('Subscription item ID (for existing items)'),
        price: z.string().describe('Price ID'),
  ...
  async ({ subscription_id, items, proration_behavior, metadata }) => {
    ...
    const subscription = await stripeRequest(`/subscriptions/${subscription_id}`, 'POST', body);
    return {
      content: [
        {
          type: 'text',
...
server.registerTool(
  ...
  async ({ subscription_id, invoice_now, prorate }) => {
    ...
    const subscription = await stripeRequest(`/subscriptions/${subscription_id}`, 'DELETE', body);
    return {
      content: [
        {
          type: 'text',
...
server.registerTool(
  'list_invoices',
  {
    ...
    inputSchema: {
      ...
      limit: z.number().optional().default(10).describe('Number of invoices to return (1-100)'),
...
server.registerTool(
  'list_products',
  {
    ...
    inputSchema: {
      ...
      limit: z.number().optional().default(10).describe('Number of products to return (1-100)'),
...
server.registerTool(
  'list_prices',
  {
    ...
    inputSchema: {
      ...
      active: z.boolean().optional().describe('Filter by active status'),
      limit: z.number().optional().default(10).describe('Number of prices to return (1-100)'),
...
server.registerTool(
  'create_price',
  {
    ...
    inputSchema: {
      ...
      }).optional().describe('Recurring billing settings'),
      metadata: z.record(z.string()).optional().describe('Set of key-value pairs for custom metadata')
    }
  },
...
server.registerTool(
  'list_refunds',
  {
    ...
    inputSchema: {
      ...
      payment_intent: z.string().optional().describe('Filter by payment intent ID'),
      limit: z.number().optional().default(10).describe('Number of refunds to return (1-100)'),
...
server.registerTool(
  'list_balance_transactions',
  {
    ...
    inputSchema: {
      limit: z.number().optional().default(10).describe('Number of transactions to return (1-100)'),
