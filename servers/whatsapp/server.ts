...
server.registerTool(
  'send_text_message',
  {
    ...
    inputSchema: {
      recipient: z.string().describe('Recipient phone number in international format (e.g., +1234567890)'),
      text: z.string().describe('The text message to send'),
      preview_url: z.boolean().optional().describe('Whether to show URL preview in the message')
    }
  },
...
server.registerTool(
  'send_template_message',
  {
    ...
    inputSchema: {
      ...
      parameters: z.array(z.string()).optional().describe('Array of parameter values for the template')
    }
  },
...
server.registerTool(
  'send_media_message',
  {
    ...
    inputSchema: {
      ...
      media_id: z.string().optional().describe('Media ID from WhatsApp (alternative to URL)'),
      caption: z.string().optional().describe('Caption for the media (for image, video, document)'),
...
server.registerTool(
  'send_interactive_buttons',
  {
    ...
    inputSchema: {
      ...
      })).min(1).max(3).describe('Array of 1-3 buttons')
    }
  },
    ...
    inputSchema: {
      ...
      sections: z.array(z.object({
        ...
        rows: z.array(z.object({
          ...
          description: z.string().optional().describe('Row description (max 72 characters)')
        }))
...
server.registerTool(
  'list_message_templates',
  {
    ...
    inputSchema: {
      limit: z.number().optional().describe('Number of templates to return (default: 100)'),
...
server.registerTool(
  'upload_media',
  {
    ...
    inputSchema: {
      ...
      media_type: z.string().describe('MIME type of the media (e.g., image/jpeg, video/mp4)')
    }
  },
