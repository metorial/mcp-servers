  ...
  'send_email',
  {
    ...
    inputSchema: {
      ...
      tags: z.array(z.object({
        name: z.string(),
        value: z.string()
      })).optional().describe('Email tags for categorization'),
...
server.registerTool(
  ...
  async ({ audience_id, ...contactData }) => {
    const result = await resendRequest(`/audiences/${audience_id}/contacts`, 'POST', contactData);
    return {
      content: [
        {
          type: 'text',
...
server.registerTool(
  ...
  async ({ audience_id, ...emailData }) => {
    const result = await resendRequest(`/audiences/${audience_id}/broadcasts`, 'POST', emailData);
    return {
      content: [
        {
          type: 'text',
